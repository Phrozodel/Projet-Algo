...
PACKAGE BODY Gestion_Guildes IS
   ...
   PROCEDURE Newguilde (G : OUT T_Guilde) IS
   ...
   BEGIN
      ...
      Put(G.Nomg(1..K));
      Put(
         " est officiellement reconnue comme une guilde Poke, devenez les meilleurs!");
      New_Line;
      Put(G.Nomg(1..K));
      Put(" possede");
      Put(G.Nb_Membres,3);
   ...
   FUNCTION Guilde_Existe (Liste_G : IN T_PtG; Guilde : IN T_Guilde) RETURN Boolean IS
   ...
   PROCEDURE Insertion_ListeG (Liste_G : IN OUT T_PtG; Guilde : IN T_Guilde) IS
   ...
   PROCEDURE Dissolution (Liste_G : IN OUT T_PtG; Guilde : IN T_Guilde) IS

   BEGIN
      IF Guilde_Existe (Liste_G,Guilde) THEN
      IF Liste_G.Guilde.NomG = Guilde.NomG THEN Liste_G:=Liste_G.SuivG;
      Put_Line("La guilde a ete dissoute, mais ne desesperez pas! Apprenez de vos erreurs, et le futur brillera pour vous ");
      END IF;
      ELSE
      Put_Line("La guilde n'a pas ete trouvee dans la liste, elle ne peut donc pas etre supprimee");
      END IF;
   ...
   PROCEDURE VisuG (G: IN T_Ptg) IS
   p : T_PtG := G;
   BEGIN
      IF p =NULL  THEN Put_Line("Il n'y a pas de guildes a ce moment...");
      ELSE
         WHILE P/=NULL LOOP
            Put("Nom de la guilde: ");
            Put(P.Guilde.Nomg);
            New_Line;
            Put("Nombre de membres: ");
            Put(P.Guilde.Nb_Membres,3);
            New_Line;
            Put("Points de la guilde:");
            Put(P.Guilde.Pointsg,3);
            New_Line;
